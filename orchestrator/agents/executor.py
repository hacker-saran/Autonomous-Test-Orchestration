...
control everything below without touching the generated file's code:
  ...
  - `ORCH_SCREENSHOT_PATH`, where that fixture writes a full-page screenshot
    ...
    `screenshot_path` — captured unconditionally (not just on failure) at a
    predictable per-flow path under dashboard/screenshots/, so the live
    dashboard can show what page a given test actually exercised even when
    it passed.
    ...
    def run(
        ...
        for gt in generated_tests:
            ...
            logger.info(
                ...
                gt.flow_id, status, duration_ms, len(console_errors), len(network_errors), result.screenshot_path,
            )
